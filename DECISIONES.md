...
## Calibración con datos reales (Etapa 2)

63. **Harness de calibración uniforme en los tres paquetes**: Cada paquete (`cfdi-parser`, `bank-parser`, `tax-engine`) tiene un directorio `tests/fixtures/reales/` excluido de git y un script `scripts/calibrar_contra_reales.py` que recorre ese directorio, corre el parser correspondiente, e imprime un reporte estructurado. Si el directorio está vacío, imprime "no hay fixtures reales para calibrar" y sale limpio (exit 0). Esto permite a Francisco soltar un archivo y correr un solo comando para calibrar, sin tocar código.

64. **`mercado_pago_real.txt` renombrado a `mercado_pago_sintetico.txt`**: El nombre `_real` era engañoso — el archivo contiene un titular fabricado (`MARIA GUADALUPE HERNANDEZ CASTRO`) y una CLABE de relleno. El riesgo es que alguien lo lea como calibración completada cuando no lo es. Se agregó un comentario al inicio del archivo documentando que es sintético. Se verificó con `grep` que no quedan otros fixtures con `_real` en el nombre cuyo contenido sea fabricado.

65. **No se fabrican evals conversacionales**: El módulo conversacional (`chat.py`) devuelve un string fijo — no hay modelo al que evaluar. Fabricar 30-50 evals produciría una ilusión de cobertura. En su lugar, el harness de evals queda estructurado (`apps/api/tests/evals/`) con 5 casos marcados explícitamente como `es_ejemplo_estructural=True`. Los evals reales se escribirán cuando haya un proveedor de IA conectado y consultas reales de contribuyentes.

66. **Tests de robustez para entradas malformadas**: Se agregaron tests que cubren XML que no es CFDI (HTML, RSS, SOAP), CFDI versión 3.3, XML truncado, archivo binario con extensión .xml, y mezcla de archivos válidos e inválidos. Para bank-parser: texto vacío, formato no reconocido, y encabezado incompleto. Estos tests usan datos sintéticos deliberadamente malformados — es apropiado fabricarlos porque prueban manejo de errores, no extracción de datos correctos.

## Capa de sesión y permisos (Etapa 3)

67. **`TenantProvider` como contexto React en vez de hook con query directa**: El hook original `useTenant()` usaba `.maybeSingle()` en la tabla `memberships`, lo cual falla cuando un usuario tiene >1 membresía (ej: un contador con varios clientes). Se reemplazó por un `TenantProvider` que carga todas las membresías con JOIN a `tenants`, expone `tenant`, `tenants[]`, `loading` y `switchTenant()`. La selección activa se persiste en `localStorage` bajo la clave `orkesta-active-tenant`.

68. **`usePermissions()` deriva booleanos de capacidad a partir del rol, no flags en BD**: En vez de almacenar permisos granulares en la tabla `memberships`, se derivan en el frontend a partir del enum `rol_usuario`. `propietario` tiene todo; `contador` tiene todo excepto `canDelete` y `canManageEfirma`; `lectura` no tiene ningún permiso de escritura. Esto evita la complejidad de una tabla `permisos` y es coherente con la semántica de los tres roles definidos en la especificación.

69. **Acciones deshabilitadas con tooltip, nunca ocultas**: La especificación dice "mostrar estado deshabilitado con tooltip, nunca ocultar acciones". Todas las acciones de escritura (subir, eliminar, calcular, invitar, guardar configuración) se muestran al rol `lectura` como botones deshabilitados con un `<Tooltip>` que explica la restricción. Para botones disabled que necesitan tooltip, se envuelven en un `<span tabIndex={0}>` porque los elementos disabled no disparan eventos de hover en algunos navegadores.

70. **Documentos como punto de entrada canónico para carga**: El wizard en `/dashboard/documentos` es el punto de entrada único para toda carga de archivos. Los botones "Subir CFDIs" en `/cfdis` y "Subir extracto" en `/extractos` redirigen al wizard en vez de implementar su propia lógica de upload. Esto centraliza la validación de archivos, las reglas de tamaño máximo (10MB), y las extensiones permitidas.

71. **RFC y régimen son read-only en la cuenta — vienen de la constancia**: La página `/dashboard/cuenta` ya no permite editar RFC ni régimen fiscal manualmente. Ambos campos se muestran como inputs deshabilitados o badges, con texto explicativo de que se derivan de la Constancia de Situación Fiscal. La re-carga de constancia se hace desde el wizard de documentos. Si la constancia contradice al perfil guardado, gana la constancia.

72. **RLS gap en `perfiles_obligacion` corregido en migración 00010**: La tabla `perfiles_obligacion` (creada en migración 00007) es un catálogo público sin `tenant_id`, pero no tenía `ENABLE ROW LEVEL SECURITY` ni política de SELECT. Se agregó en migración 00010 siguiendo el mismo patrón que `ejercicios`, `tarifas_resico` y las tablas del catálogo normativo.
